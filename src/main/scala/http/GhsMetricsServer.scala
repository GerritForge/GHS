...
import http.GhsMetricsServer.{AUTHORIZATION_HEADER, shouldUseCompression}
...
/**
 * A copy of the [[kamon.prometheus.embeddedhttp.SunEmbeddedHttpServer]] but
 * with Authorization checking capabilities.
 */
class GhsMetricsServer(hostname: String, port: Int, path: String, scrapeSource: ScrapeSource, config: Config) extends EmbeddedHttpServer(hostname, port, scrapeSource, config) {
  private val server = {
    ...
    val handler = new HttpHandler {
      override def handle(httpExchange: HttpExchange): Unit = {
        ...
        val authorizationHeaderValue = httpExchange
          .getRequestHeaders
          .asScala
